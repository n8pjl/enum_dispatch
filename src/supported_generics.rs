...
impl std::fmt::Display for UnsupportedGenericArg {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::NonIdentifierType => write!(
                f,
                "Generic types in #[enum_dispatch(...)] must be identifiers"
            ),
            Self::NonIntegralConstGenericType => write!(
                f,
                "Non-integral const generic types in #[enum_dispatch(...)] are not supported"
            ),
            Self::Lifetime => write!(
                f,
                "Lifetime generics in #[enum_dispatch(...)] are not supported"
            ),
            Self::AssocType => write!(
                f,
                "Generic associated types in #[enum_dispatch(...)] are not supported"
            ),
            Self::AssocConst => write!(
                f,
                "Generic associated constants in #[enum_dispatch(...)] are not supported"
            ),
            Self::Constraint => write!(
                f,
                "Generic trait constraints in #[enum_dispatch(...)] are not supported"
            ),
            Self::Unknown => write!(
                f,
                "Unsupported generic argument syntax in #[enum_dispatch(...)]"
            ),
        }
    }
}
...
const SUPPORTED_CONST_GENERIC_TYPES: &[&str] = &[
    "u8", "i8", "u16", "i16", "u32", "i32", "u64", "i64", "u128", "i128", "usize", "isize", "char",
    "bool",
];
...
/// it is not supported.
pub fn convert_to_supported_generic(
    generic_arg: &syn::GenericArgument,
) -> Result<SupportedGenericArg, (UnsupportedGenericArg, proc_macro2::Span)> {
    ...
    match generic_arg {
        ...
        syn::GenericArgument::Const(syn::Expr::Lit(syn::ExprLit { attrs: _, lit })) => match lit {
        ...
        syn::GenericArgument::Const(_) => {
            Err((UnsupportedGenericArg::NonIntegralConstGenericType, span))
        }
